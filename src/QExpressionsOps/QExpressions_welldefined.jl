...
Returns a vector of vectors of booleans. Each inner vetor specifies which of its subsystem indices are acted upon by the QObj. 
...
function which_ensemble_acting!(q::QAtom, accum::Vector{BitVector}, subspace_info::SubSpaceInfo, neutral_ensembles_op::Vector{Vector{Is}}; do_abstract::Bool=false)::Vector{BitVector}
    for (i, (inds, neutral)) in enumerate(zip(subspace_info.ensemble_indices, neutral_ensembles_op))
...
Check, which Summation indices are present. Returns a Boolean of 
"""
function which_summations_acting(q::QObj, subspace_info::SubSpaceInfo)::Vector{BitVector}
    ...
    which_summations_acting(q, empty_vec, subspace_info.summation_indices)
end
...
function which_summations_acting(q::QAtomProduct, where_acting::Vector{BitVector}, which_indices::Vector{Vector{Int}})
    return Vector{BitVector}()
end
function which_summations_acting(q::QExpr, where_acting::Vector{BitVector}, which_indices::Vector{Vector{Int}})::Vector{BitVector}
    for t in q.terms
        vecvec_or!(where_acting, which_summations_acting(t, where_acting, which_indices))
    end
    return where_acting
end
function which_summations_acting(q::T, where_acting::Vector{BitVector}, which_indices::Vector{Vector{Int}})::Vector{BitVector} where T <: QComposite 
    return which_summations_acting(q.expr, where_acting, which_indices)
end
function which_summations_acting(q::T, where_acting::Vector{BitVector}, which_indices::Vector{Vector{Int}})::Vector{BitVector} where T <: QMultiComposite
    for x in q.expr
        vecvec_or!(where_acting, which_summations_acting(x,  where_acting, which_indices))
    end
    return where_acting 
end
function which_summations_acting(q::AbstractQSum, where_acting::Vector{BitVector}, which_indices::Vector{Vector{Int}})::Vector{BitVector}
    which_summations_acting(q.expr, where_acting, which_indices)
    info = q.qspace.subspace_info
...
function gather_summation_indices!(q::AbstractQSum, curr_inds::Vector{SubSpaceIndex}=SubSpaceIndex[])::Vector{SubSpaceIndex}
    for index in iter_all_indices(q)
        sorted_push_unique!(curr_inds, index)
    end 
    return curr_inds
end
...
""" 
    are_indices_defined(q::diffQEq)::Bool

Checks if all indices n the differential equation are properly specified, either by the left-hand-side or 
by AbstractQSums on the right-hand-side.
"""
function are_indices_defined(q::QAtomProduct, where_defined::Vector{BitVector})::Bool
...
function are_indices_defined(q::QExpr, where_defined::Vector{BitVector})::Bool
    # check element wise if all indices are defined
    return all([are_indices_defined(t, where_defined) for t in q.terms])
end
function are_indices_defined(q::T, where_defined::Vector{BitVector})::Bool where T <:QComposite
    return are_indices_defined(q.expr, where_defined)
end
function are_indices_defined(q::T, where_defined::Vector{BitVector})::Bool where T <:QMultiComposite
    return all([are_indices_defined(t, where_defined) for t in q.exprs])
end
function are_indices_defined(q::AbstractQSum, where_defined::Vector{BitVector})::Bool
    info = q.qspace.subspace_info
